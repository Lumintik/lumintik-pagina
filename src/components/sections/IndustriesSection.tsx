...
export function IndustriesSection() {
  ...
  return (
    <section id="industries" className="relative w-full py-14 md:py-24 z-[2] overflow-hidden">
      <div className="px-5 md:px-12">
        <div className="mx-auto max-w-[1600px]">
          <p className="text-xs md:text-sm font-medium text-blue-600">{copy.eyebrow}</p>
          <h2 className="mt-3 text-slate-900 text-4xl md:text-6xl font-semibold leading-[1.05] max-w-[18ch]">
            {copy.title} <span className="italic text-blue-600">{copy.titleAccent}</span>
          </h2>
          <p className="mt-5 text-slate-500 text-base md:text-xl leading-relaxed max-w-[56ch]">{copy.intro}</p>
        </div>
      </div>
      <Carousel items={cards} labels={{ previous: copy.previous, next: copy.next, close: copy.close }} />
    </section>
  );
}
