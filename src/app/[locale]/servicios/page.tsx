...
export async function generateMetadata({ params }: PageProps<"/[locale]/servicios">): Promise<Metadata> {
  ...
  const description = t.pages.services.intro;
  return {
    title,
    description,
    alternates: localizedAlternates(locale, paths.services),
    openGraph: {
      type: "website",
      siteName: SITE_NAME,
      title,
      description,
      url: absoluteUrl(locale, paths.services),
...
export default async function Page({ params }: PageProps<"/[locale]/servicios">) {
  const locale = fromSegment((await params).locale);
  if (!locale) notFound();
  return <ServicesPage />;
}
